...
public class RequestHandler {
	...
	public static void onGET(BufferedOutputStream os, String string, HashMap<String, String> uRL_args,
		...
		if (FileSharing.isDebugEnabled())
			FileSharing.getLogger().log(Level.INFO, "[DEBUG] onGet() HTTP_Body " + HTTP_body);
...
//					writeHeader("application/octet-stream", info.getSize(), os);
	...
	private static HashMap<String, String> readHTTP_body(InputStream is, AtomicLong expectedDataLen) throws IOException {
		...
		while((row = readLine(is, expectedDataLen)) != null && row.length() > 2) {
			...
			if (subArgs.length != 2) {
				if (FileSharing.isDebugEnabled()) FileSharing.getLogger().log(Level.INFO, "readHTTP_body() invalid while -> " + row);
				continue;
			}
	...
	public static void onPOSTmultipart(BufferedOutputStream os, String string, HashMap<String, String> URL_args, HashMap<String, String> HTTP_body, BufferedInputStream is) throws IOException {
		
		if (FileSharing.isDebugEnabled()) 
			FileSharing.getLogger().log(Level.INFO, "[DEBUG] HTTPBody: " + HTTP_body);
		...
		if (FileSharing.isDebugEnabled()) 
			FileSharing.getLogger().log(Level.INFO, "[DEBUG] PostBody: " + postBody);
		...
		String contentType = postBody.get("content-type");
